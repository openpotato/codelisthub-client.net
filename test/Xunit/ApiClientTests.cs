...
public class ApiClientTests
{
    ...
    public async Task TestGetDocument()
    {
        ...
        var document = await client.GetDocumentAsync(new Uri("urn:codelisthub:iso:countries:v1"), false, cancellationToken: TestContext.Current.CancellationToken);
        ...
        if (document is CodeListDocument codeListDocument)
        {
            ...
            var nonLocalizedString = Assert.IsType<NonLocalizedString>(codeListDocument.Columns[0].Name);
            Assert.Equal("Alpha 2 Schlüssel", nonLocalizedString.Value);
    ...
    public async Task TestGetDocumentIndex()
    {
        ...
        var documentIndex = await client.GetDocumentIndexAsync(DocumentType.CodeList, "ISO", null, null, null, null, 1, 20, TestContext.Current.CancellationToken);
