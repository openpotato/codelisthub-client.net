...
namespace CodeListHub.Client
{
    ...
    public class ApiClient : ApiBaseClient
    {
        ...
        /// <param name="cancellationToken">The token to monitor for cancellation requests.</param>
        /// <returns>A task that represents the asynchronous operation. The value of the TResult parameter 
        /// contains a <see cref="Document"/> instance.</returns>
        ...
        public async Task<Document> GetDocumentAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            return await DocumentLoader.LoadAsync(await GetDocumentAsStreamAsync(canonicalUri, metaOnly, language, cancellationToken), cancellationToken);
        }
